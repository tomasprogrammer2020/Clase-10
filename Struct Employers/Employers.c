...
float maxSueldo (eEmpleado lista[], int tam)
{
    int i;
    float max=-1;
    int flag=0;
    ...
    for (i=0; i<tam; i++)
    {
        if (flag==0 || lista[i].sueldoBruto>max)
        {
            max=lista[i].sueldoBruto;
            flag=1;
        }
    }
...
void mostrarSueldoMax(eEmpleado lista[], int tam)
{
    int i;
    float max;
    max=maxSueldo(lista, tam);

    if (max!=-1)
    {
        for (i=0; i<tam; i++)
    {
        if (lista[i].sueldoBruto==max)
        {
          mostrarEmpleado(lista[i]);
        }
    }
...
void mostrarCarlos(eEmpleado lista[], int tam)
{
    int i;
    int indiceCarlos=-1;
    ...
    for (i=0; i<tam; i++)
    {
        ...
        if (lista[i].sueldoBruto>20.000 && strcmp(lista[i].nombre, "Carlos")==0)
        {
            mostrarEmpleado(lista[i]);
        }
    }
